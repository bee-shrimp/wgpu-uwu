...
impl Renderer {
    ...
    pub fn render(&mut self) {
        ...
        // ---------------------------------------------------------------------------------------- renderpass for l1
        let mut l1_renderpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("l1 renderpass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: &self.l1_texture_view,
                depth_slice: None,
                resolve_target: None,
                ops: wgpu::Operations {
                    load: wgpu::LoadOp::Clear(wgpu::Color::TRANSPARENT),
        ...
        // ------------------------------------------------------------------------------------------- use the renderpass
        l1_renderpass.set_pipeline(&self.l1_render_pipeline);
        l1_renderpass.set_bind_group(0, Some(&self.l1_bind_group), &[]);
        l1_renderpass.set_vertex_buffer(0, self.fullscreen_vertex_buffer.slice(..));
        l1_renderpass.draw(0..3, 0..1);
